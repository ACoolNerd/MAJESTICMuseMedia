...
import { sourceRegistry } from '../features/intelligence/seed';
...
import { callTrustedApi, trustedServerConfigured } from '../lib/api';
...
export default function MuseIntelligence() {
  ...
  const serverMode = trustedServerConfigured && Boolean(user && !user.isDemo);
  ...
  async function run(input: string) {
    ...
    try {
      ...
      const sourceContent: Record<string, string> = {
        'production-control-center': JSON.stringify({ episodes, guests }),
        'master-operating-source': JSON.stringify({
          governance: {
            ceo: 'Marchette retains final brand, editorial, guest, partnership, release and publishing approval.',
            coo: 'Keith McPherson / ACoolNERD manages technology, AI operations, administration and integrations without overriding final editorial authority.',
            post: 'Brian Schwager owns post-production and technical quality.',
            social: 'Ares Schwager owns approved distribution and community operations.',
          },
          truthRules: ['Proposed and TBD are not Confirmed.', 'AI may recommend but cannot publish, send, approve, clear rights or confirm records.'],
        }),
        'master-operating-workbook': JSON.stringify({ assets, comments, connections, jobs }),
      };
      const enabledSources = sourceRegistry
        .filter(source => sourceContent[source.id])
        .map(source => ({
          id: source.id,
          title: source.title,
          ...
          content: sourceContent[source.id]!,
        }));
      const response = await callTrustedApi<ServerMuseResponse>('/api/muse/answer', {
        method: 'POST',
        body: JSON.stringify({ question: input, sources: enabledSources }),
      });
      setAnswer({
        ...
        citations: response.citedSourceIds.map(sourceId => {
          const source = sourceRegistry.find(item => item.id === sourceId);
          return {
            sourceId,
            sourceTitle: source?.title ?? sourceId,
            status: source?.status ?? 'System Evidence',
            observedAt: source?.updatedAt ?? new Date().toISOString(),
          };
        }),
        generatedAt: new Date().toISOString(),
        mode: 'Gemini structured output',
        requiresApproval: response.requiresApproval,
      });
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Muse Intelligence failed.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return <AppShell>
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
      <section className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-[#1B1734]/5">
        <h2 className="font-serif text-2xl">Starter questions</h2><div className="mt-4 space-y-2">{starterQuestions.map(item => <button key={item} onClick={() => void run(item)} disabled={loading} className="w-full rounded-2xl border border-[#1B1734]/8 px-4 py-3 text-left text-sm hover:border-[#8B2C6F] disabled:opacity-50">{item}</button>)}</div>
        <form onSubmit={submit} className="mt-6 border-t border-[#1B1734]/10 pt-6"><label className="text-sm font-semibold">Ask Muse<textarea value={question} onChange={event => setQuestion(event.target.value)} rows={4} className="mt-2 w-full rounded-2xl border border-[#1B1734]/15 p-4 outline-none focus:border-[#8B2C6F]"/></label><button disabled={loading} className="mt-3 w-full rounded-full bg-[#1B1734] px-5 py-3 font-semibold text-white disabled:opacity-50">{loading ? 'Generating grounded answer…' : 'Generate grounded answer'}</button></form>
      </section>
      ...
      <section className="rounded-[2rem] bg-[#1B1734] p-6 text-white shadow-xl">
        {!answer ? <div className="flex min-h-[480px] flex-col items-center justify-center text-center"><Sparkles className="text-[#C9A227]" size={52}/><h2 className="mt-5 font-serif text-3xl">Muse is ready.</h2><p className="mt-3 max-w-md text-white/60">Choose a question to inspect the loaded operating records without inventing completion.</p></div> : <div>
          ...
          <p className="mt-5 flex items-center gap-2 text-xs text-white/45"><CheckCircle2 size={14}/>{answer.mode}. Recommendations remain reviewable and do not publish, send or confirm records automatically.</p>
        </div>}
      </section>
    </div>
  </AppShell>;
